...
{-e is bound to a value of bool, which is either true or false. r is

bound to S (S t) and t is bound to the value of S (S O). (S O) is the

successor of zero which is one. Therefore, r is equivalent to

three. when the function evenb (S r), it means you are asking if the

numerical value three is even, so you get the boolean

value of false. therefore when the function e is put in the input, the
